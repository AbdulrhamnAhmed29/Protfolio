import {  MousePointerClick,  Code,   Award, Download,  TrendingUp, Briefcase, Mail } from "lucide-react";
...
export const HeroSection = () => {
  ...
  const codeSnippets = [
    "import { ReactDeveloper } from 'AbdulrhmanAhmed';",
    "",
        ...
        <motion.div className="flex flex-col lg:flex-row items-center justify-between gap-16 lg:gap-20" initial="hidden" animate={isInView ? "visible" : "hidden"} variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.25, delayChildren: 0.5 } } }}>
          ...
          <div className="flex-1 text-center lg:text-left max-w-2xl mx-auto lg:mx-0">
            ...
            <motion.h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-tight tracking-tight" variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
              <span className="block text-foreground">I'm Abdulrhman</span>
              <motion.span className="block bg-gradient-to-r from-primary via-purple-600 to-pink-600 bg-clip-text text-transparent mt-2" animate={{ backgroundPosition: ['0%', '100%', '0%'] }} transition={{ duration: 8, repeat: Infinity }} style={{ backgroundSize: '200% 100%' }}> Front End Developer
              </motion.span>
            </motion.h1>

            <motion.p className="text-lg mb-5 sm:text-xl text-muted-foreground mt-6 leading-relaxed max-w-2xl" variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
              I build <span className="text-primary font-semibold">high-performance web applications</span> that drive business growth. Specializing in React and scalable architecture for startups and enterprises.
            </motion.p>
