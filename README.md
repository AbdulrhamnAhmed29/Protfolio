# 👨‍💻 Abdulrhman Ahmed — Frontend Developer

Welcome to my portfolio 🚀
I build modern, responsive, and high-performance web applications using the latest frontend technologies.

---

## 🧠 About Me

I'm a passionate **Frontend Developer** focused on building clean UI, smooth UX, and scalable web applications.

* 💡 Strong in JavaScript & React
* 🎯 Focused on performance & clean code
* 🔥 Always learning and building real-world projects

---

## ⚙️ Tech Stack

### 🖥️ Frontend

* HTML5
* CSS3 / Tailwind CSS
* JavaScript (ES6+)
* TypeScript
* React.js

### 🛠️ Tools & Others

* Git & GitHub
* Postman
* REST APIs
* GitHub Copilot

---

## 🚀 Featured Projects

### 🍽️ SOUL KITCHEN

Modern restaurant & e-commerce platform with reservation system.

**Tech:** React, Laravel, API Integration

📂 Repo: *https://github.com/AbdulrhamnAhmed29/SoulKitchen*

---

### 🚗 Elvoram Motors

Car showcase platform with filtering system & admin dashboard.

**Tech:** React, Laravel
📂 Repo: *https://github.com/AbdulrhamnAhmed29/Elvora-Motors*

---


## 📫 Contact Me

* 💼 LinkedIn: *https://www.linkedin.com/in/abdulrhman-aahmed-828a94354/*
* 📧 Email: *abdulrhmanaahmed4@gmail.com*

---

## 🔥 What I'm Working On

* Building advanced React projects
* Learning Next js 
*Learning Sql and data base schmea 
*Learning Ai Automation 


---

⭐ If you like my work, feel free to star my repos!
